...
# 
# This script performs the linpack benchmark
#
...
function compile_linpack {
    cd $LINPACK_DIR
    echo 'Iam at: "'$LINPACK_DIR'"' # TODO remove
    LINPACK_MAKE=Make.${LINPACK_ARCH}
    aux=$(sed 's/\//\\\//g' <<< $(pwd))
    sed 's/$(HOME)\/hpl/'$aux'/g' ../setups/${LINPACK_MAKE} \
        > ../setups/${LINPACK_MAKE}.new
    sed 's/\/opt\/cesga\/.*libopenblas\.so/\/usr\/lib64\/libblas.so/g'\
        ../setups/${LINPACK_MAKE}.new > ../setups/${LINPACK_MAKE}
    mv ../setups/${LINPACK_MAKE} Make.${LINPACK_ARCH}
